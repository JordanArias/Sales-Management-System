...
export class AppComponent {
  ...
  usuario=[{ci_usuario: '', clave: '',nom_usu:'',ap_usu:''}];
  gestion='';
  Ausuario=false; Aproducto=false; Aventa=false; Acaja=false; Acocina=false;
  classUsu=false; classProd=false; classVenta=false; classCaja=false; classCocina=false;
    ...
    this.authService.getUsuarioRoles(id)
      .subscribe(
        res => {
          ...
          this.Ausuario=false; this.Aproducto=false; this.Aventa=false; this.Acaja=false; this.Acocina=false;
          this.tieneRolNecesario();
        },
...
//Efecto del componente seleccionado para que se remarque
  ...
  seleccionarMenu(opcion: any): void {  
    if (opcion==1) {this.gestion='Gestion Usuarios';}
    if (opcion==2) {this.gestion='Gestion Productos';}
    if (opcion==3) {this.gestion='Gestion Ventas';}
    if (opcion==4) {this.gestion='Gestion Caja';}
    if (opcion==5) {this.gestion='Gestion Cocina';}
